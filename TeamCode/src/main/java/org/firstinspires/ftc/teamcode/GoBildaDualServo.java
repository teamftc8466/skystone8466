...
public class GoBildaDualServo {
    ...
    // 0 pos = 0 degrees, 1 pos = MAX_SEVOR_MODE_DEGREE degrees
    void setServoModePositionInDegree(double degree,
                                      boolean show_position_info) {
        if (degree >= MAX_SEVOR_MODE_DEGREE) {
            servo_.setPosition(1);                                  // If the inputted number of degrees exceeds the range of the servo, automatically set the servo position to the max
        } else if (degree <= 0) {
            servo_.setPosition(0);                                  // If the inputted number of degrees is negative, automatically set the servo position to the minimum
        } else {
            double position = degree / MAX_SEVOR_MODE_DEGREE;       // Conversion from the user-inputted number of degrees (the variable degree) to the servo position scale equivalent (the variable servo280position_), i.e. input is 140 degrees, the servo will be set to 0.5 position because 140*(1/280) = 0.5)
